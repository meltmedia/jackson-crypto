...
public class EncryptWithObjectMapperTest {
	...
	public void shouldEncryptString() throws IOException {
		WithEncrypted toEncrypt = new WithEncrypted()
		  .withStringValue("some value");
		...
		assertThat( "nested value is encrypted", encrypted.at("/stringValue/value").asText(), not(containsString("some value")));
	...
	public void shouldEncryptInteger() throws IOException {
		...
		assertThat( "nested value is encrypted", encrypted.at("/intValue/value").asText(), not(containsString("10")));
	}
	
	@Test
	public void shouldEncryptNested() throws IOException {
		WithEncrypted toEncrypt = new WithEncrypted()
		  .withNestedValue(new Nested().withValue("some value"));
		
		ObjectNode encrypted = mapper.readValue(mapper.writeValueAsString(toEncrypt), ObjectNode.class);

		assertThat( "has nested value", encrypted.at("/nestedValue/value").isNull(), equalTo(false));
		assertThat( "nested value is encrypted", encrypted.at("/nestedValue/value").asText(), not(containsString("some value")));
	}
	
	@Test
	public void shouldEncryptList() throws IOException {
		WithEncrypted toEncrypt = new WithEncrypted()
		  .withListValue(Arrays.asList("value1", "value2"));
		
		ObjectNode encrypted = mapper.readValue(mapper.writeValueAsString(toEncrypt), ObjectNode.class);

		assertThat( "has nested value", encrypted.at("/listValue/value").isNull(), equalTo(false));
		assertThat( "nested value is encrypted", encrypted.at("/listValue/value").asText(), not(containsString("value")));
	}
	...
	@Test
	public void shouldEncryptWithSerializerAnnotation() throws JsonParseException, JsonMappingException, JsonProcessingException, IOException {
